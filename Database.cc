...
void Database::merge_buffer(Buffer b) {
  ...
  FileInfo file_info(filename, 0, fp);
  BloomFilter bf(bloom_filter_bits, calculate_hashes(bloom_filter_bits, file_length));
...
void Database::merge(int level_number) {
  ...
  BloomFilter bf(bloom_filter_bits, calculate_hashes(bloom_filter_bits, file_length * (int)std::pow(runs_per_level, level_number+1)));
  RunInfo run_info(bf);
  while (true) {
